...
const allPlayerStateUpdatesArePresent = (events) => {
  ...
  for (let i = 0; i < events.length; i++) {
    const e = events[i];
    if (Array.isArray(e.message) && /\w+: [0-9]* amber \w+: [0-9]* amber/.test(e.message[0])) {
      const matches = e.message[0].match(/(\w+): ([0-9]*) amber (\w+): ([0-9]*) amber/);
...
const getTotalAemberGained = (player, events) => {
  ...
  // TODO consider amount lost, stolen, and captured?
  let total = 0;
...
const getAemberCaptured = (player, events) => {
  ...
  events.forEach(e => {
    ...
    if (capturedAember) {
      ...
      if (/^cause each undamaged creature to capture [0-9]* amber from their opponent$/.test(text)) {
        ...
        console.log('Aember capture issue:', text);
      }

      if (isNaN(amount)) {
        console.log(e.message[4].message[0]);
      }
    }
  });
...
const getAemberGainedFromPipsForPlayer = (player, events) => {
  const aemberPipsGainedEvents = events.filter(event => {
    return (
...
const getAemberGainedFromReapForPlayer = (player, events) => {
  ...
  const reapEvents = events.filter(event => {
    return (
      Array.isArray(event.message)
      && event.message[0].name === player
  ...
  const crystalHiveEvents = events.filter(event => {
    return (
      Array.isArray(event.message)
      && event.message[0].name === player
