...
const analyze = (events) => {
  ...
  const firstPlayer = events.find(event => {
    return (
      event.message
      && event.message.alert
      && event.message.alert.type === 'endofround'
    );
  }).message.alert.message[2].name;
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (Array.isArray(event.message) && /\w+: [0-9]* amber \w+: [0-9]* amber/.test(event.message[0])) {
      const matches = event.message[0].match(/(\w+): ([0-9]*) amber (\w+): ([0-9]*) amber/);
