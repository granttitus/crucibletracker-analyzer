...
const getChainsGained = (player, events) => {
  const chainReducedEvents = events.filter(event => {
    return (
...
const getStartingChains = (player, events) => {
  const endOfTurn1Index = findLastIndex(events, (event) => {
    return (
      event.message
      && event.message.alert
  ...
  const firstChainReduction = events.slice(0, endOfTurn1Index).find(event => {
    return (
      Array.isArray(event.message)
      && event.message[0].name === player
      && /s chains are reduced by 1 to /.test(event.message[1])
  ...
  if (firstChainReduction) {
    return firstChainReduction.message[2] + 1;
  } else {
    return 0;
  }
};
